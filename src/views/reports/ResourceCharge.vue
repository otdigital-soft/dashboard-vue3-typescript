<template>
  <div class="px-4 sm:px-6 lg:px-8">
    ...
    <div v-if="(readyToView || reportItems.length > 0) && copyGroupBy" class="mt-14">
      <div v-if="!orderBy" class="max-w-lg m-auto">
                    ...
                    <div class="absolute inset-0 px-4 sm:px-6">
                      <div class="h-full border-2 border-dashed border-gray-200 dark:border-gray-700 p-6 text-sm" aria-hidden="true">
                        <Form @submit="viewData">
                          ...
                          <div class="flex items-center mt-4">
                            <label for="aggregateBy" class="w-24 shrink-0 text-gray-500 dark:text-gray-200">Aggregate By</label>
                            <Field
                              id="aggregateBy"
                              v-model="aggregateBy"
                              name="aggregateBy"
                          ...
                          <div class="flex items-center mt-4">
                            ...
                            <div class="grow border border-dashed border-slate-400 rounded px-4 py-3">
                              <ReportFilterView report-type="reportsCharge" @add="addFilter" />
                              <div class="mt-4">
...
import ChargeGroupView from "@/components/report/ChargeGroupView.vue"
...
import { ResourceChargeGroupbyColumns as groupbyColumns } from "@/models/reports/reportColumn"
...
const { value: aggregateBy } = useField<string>("aggregateBy", undefined, {
  initialValue: ""
})
...
const handleExport = async () => {
  ...
  await ReportApi.getResourceChargeFile(top.value, groupBy.value, aggregateBy.value, orderBy.value, orderValue, filtersValue, cols, "csv")
}
...
const fetchResourceCharges = async () => {
  let orderValue: string | null
  if (orderDesc.value) {
    orderValue = orderBy.value
  } else {
  ...
  const { data } = await ReportApi.getResourceCharges(top.value, groupBy.value, aggregateBy.value, orderBy.value, orderValue, filtersValue, cols)
