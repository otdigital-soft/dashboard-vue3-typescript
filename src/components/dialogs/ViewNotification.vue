  ...
  <TransitionRoot as="template" :show="open">
    <Dialog as="div" class="relative z-10" @close="$emit('close')">
      ...
      <div class="fixed z-10 inset-0 overflow-y-auto">
        <div v-if="notificationDetail" class="flex items-end sm:items-center justify-center min-h-full p-4 text-center sm:p-0">
          <TransitionChild
            as="template"
            enter="ease-out duration-300"
              ...
              <div class="sm:flex sm:items-start">
                ...
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-200">
                    <vue-markdown :source="notificationDetail?.subject" />
                  </DialogTitle>
                  <div class="my-2">
                    <p id="notification" class="text-sm text-gray-500 dark:text-gray-200">
                      <vue-markdown :source="notificationDetail?.content" />
                    </p>
                  </div>
                </div>
              </div>
...
const { notificationDetail } = useNotification()
const { getNotification } = useNotificationStore()

watch(isOpen, (val) => {
  // if val == true, that mean the dialog is just open:
  if (val === true) {
    if (props.id !== null) {
      getNotification(props.id)
    }
  }
})
