                  ...
                  <Form @submit="savePolicy">
                    ...
                    <div class="py-2">
                      <div class="flex w-full mt-5 relative">
                        ...
                        <div class="relative">
                          <Switch
                            v-model="policyActive"
                            class="relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                            :class="policyActive ? 'bg-blue-600' : 'bg-gray-200 dark:bg-gray-400'"
                          >
                            <span className="sr-only">Use setting</span>
                            <span
                              class="pointer-events-none relative inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200"
                              :class="policyActive ? 'translate-x-5' : 'translate-x-0'"
                            >
                              <span
                                class="absolute inset-0 h-full w-full flex items-center justify-center transition-opacity"
                                :class="policyActive ? 'opacity-0 ease-out duration-100' : 'opacity-100 ease-in duration-200'"
                              />
                              <span
                                class="absolute inset-0 h-full w-full flex items-center justify-center transition-opacity"
                                :class="policyActive ? 'opacity-100 ease-in duration-200' : 'opacity-0 ease-out duration-100'"
                              />
                            </span>
                          </Switch>
                        </div>
...
const policyActive = ref(false)

const savePolicy = () => {
  if (policyName.value !== null) {
    policyApi.create(policyName.value, policyActive.value)
      .then(() => {
